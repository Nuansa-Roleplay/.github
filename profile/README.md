# 🌆NUANSA ROLEPLAY

Selamat datang di **NUANSA ROLEPLAY**, komunitas **GTA 5 Roleplay (FiveM)** yang berfokus pada **pengalaman bermain realistis**, **komunitas solid**, dan **sistem kota yang terus berkembang**.  

Kami hadir untuk membangun dunia roleplay yang **berkualitas, imersif, dan penuh cerita** — tempat setiap pemain bisa menjadi siapa pun yang mereka inginkan.

## 🚀 Tentang Kami

**NUANSA ROLEPLAY** adalah komunitas yang berdiri di atas nilai **attitude**, **komunikasi**, dan **profesionalitas**.  
Kami tidak hanya sekadar bermain — kami membangun **sebuah kota hidup**, **komunitas aktif**, dan **tempat tumbuhnya kreativitas**.

🎯 **Fokus Kami:**
- Menyediakan server yang **stabil dan adil**.  
- Menjaga **aturan dan etika roleplay**.  
- Mengembangkan **fitur dan sistem** sesuai kebutuhan warga.  
- Menumbuhkan **komunitas positif** antar pemain dan admin.  

## 🧩 Apa yang Kami Kembangkan

Repositori di organisasi ini berisi:

- ⚙️ **Server Scripts & Systems** — Pengembangan dan optimasi fitur kota.  
- 🧠 **Documentation** — Panduan, SOP, dan aturan komunitas.  
- 🧩 **Internal Tools** — Sistem manajemen, API, dan automasi server.  
- 🎨 **Design & Branding Assets** — Elemen visual Hustle RP.  

## 💼 Bergabung & Kontribusi

Kami terbuka bagi developer, staff, dan warga yang ingin ikut berkontribusi dalam pengembangan kota Hustle RP.  

### 💡 Cara Berkontribusi:
1. Fork repository terkait.  
2. Buat branch baru (`feature/nama-fitur`).  
3. Lakukan perubahan & testing.  
4. Ajukan **Pull Request** dengan deskripsi yang jelas.  

> Semua kontribusi akan direview oleh tim Hustle Developer sebelum di-merge.

## 📢 Sosial & Komunitas

- 💬 **Discord:** [Join Discord NUANSA ROLEPLAY](https://discord.gg/DdPKnW93A5)

### ✨ “Build the City, Grow the Community, and Hustle Together.”
